...
export function Email({
  ...
  const handleGoogleSignIn = (strategy: OAuthStrategy) => {
    console.log("Authenticating with strategy:", typeof strategy);
    return signIn!.authenticateWithRedirect({
      strategy,
      redirectUrl: "auth/sign_up/sso-callback",
      redirectUrlComplete: "/dashboard",
    });
  };
  return (
    <div className="flex items-center justify-center min-h-screen bg-primay">
      <motion.div
        ...
        <form onSubmit={(e) => handleEmailSubmit(e)} className="space-y-6">
          <div>
            ...
            <motion.input
              ...
              required
              className="mt-1 text-black block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            />
          </div>
          <motion.button
